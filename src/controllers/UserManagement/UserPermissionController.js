...
class UserPermissionController {
  ...
  async getAllUserPermissions(req, res) {
    try {
      const permission =
        await UserPermissionHandleService.getAllUserPermissions();
  ...
  async assignPermissionToUser(req, res) {
    try {
      const assign = await UserPermissionHandleService.assignPermissionToUser(
        req.body.user_id,
  ...
  async updateUserParmissionAssign(req, res) {
    try {
      ...
      const result =
        await UserPermissionHandleService.updateUserParmissionAssign(data);
  ...
  async getUserPermissionAssignByUserId(req, res) {
    try {
      ...
      if (!userPermissionAssign) {
        return res
          .status(200)
          .json({ message: 'No permission assigned to user' });
      }

      const result =
        await UserPermissionHandleService.getUserPermissionAssignByUserId(
          req.params.user_id
        );
      ...
      res
        .status(500)
        .json({ message: 'Error in getUserPermissionAssignByUserId' });
    }
  }
}
