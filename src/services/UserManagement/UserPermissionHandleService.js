...
class UserPermissinHandleService {
    ...
    try {
      ...
      permissionGroups.forEach((permissionGroup) => {
        response.permission_type.forEach((permissionType) => {
          if (
            permissionType.category_id.equals(
              permissionGroup.permission_category_id
            )
          ) {
      ...
      permissions.forEach((permission) => {
        response.permission_type.forEach((permissionType) => {
          permissionType.permission_grp.forEach((permissionGroup) => {
            if (
              permissionGroup.group_id.equals(permission.permission_group_id)
            ) {
        ...
        response.permission_type.forEach((permissionType) => {
          permissionType.permission_grp.forEach((permissionGroup) => {
            permissionGroup.permissions.forEach((permission) => {
              if (
                permission.permission_id.equals(permissionCheckBox.permissionId)
              ) {
  ...
  async assignPermissionToUser(userId, permission_ids) {
    try {
      const userPermissionAssign = await UserPermissionAssign.findOneAndUpdate({
        userId,
        permission_ids,
      });

      if (!userPermissionAssign) {
        const newUserPermissionAssign = new UserPermissionAssign({
          user_id: userId,
          permission_ids,
        });
  ...
  async updateUserParmissionAssign(data) {
    try {
      const permissionAssign = await UserPermissionAssign.findOneAndUpdate(
        { user_id: data.id },
  ...
  async getUserPermissionAssignByUserId(id) {
    try {
      ...
      const userPermissionAssign = await UserPermissionAssign.findOne({
        user_id: id,
      });
